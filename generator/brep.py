"""
Enhanced 3D B-rep construction with proper volume calculation
"""
import numpy as np
...
class BRepValidator:
    ...
    def validate(self, building: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check wall coverage
        perimeter = 2 * (math.sqrt(floor_area) * 2) if floor_area > 0 else 0
        wall_length = results['statistics'].get('wall_length_m', 0)
        
        if wall_length > 0:
            coverage = (wall_length / perimeter) * 100 if perimeter > 0 else 0
            results['statistics']['wall_coverage_percent'] = coverage
