...
class ProceduralLayoutSolver:
    ...
    def solve(self, adjacency_graph: nx.Graph,
        ...
        # Use recursive subdivision that maintains adjacency
        rooms = self._recursive_subdivision(building, room_list, adjacency_graph, 0)
    ...
    def _prioritize_rooms(self, room_list: List[str]) -> List[str]:
        ...
        priority_order = ['living', 'kitchen', 'dining', 'master_bedroom', 'bedroom', 'bathroom', 'hallway']
    ...
    def _recursive_subdivision(self, space: Polygon, rooms: List[str], 
                                graph: nx.Graph, depth: int) -> Dict[str, Polygon]:
        """Recursively subdivide space ensuring room adjacency"""
        ...
        # Find split point (prefer vertical or horizontal)
        split_vertical = random.choice([True, False])
        bounds = space.bounds
        width = bounds[2] - bounds[0]
        height = bounds[3] - bounds[1]
        
        # Calculate split ratio based on room areas
        split_ratio = 0.5
        
        if split_vertical:
            split_x = bounds[0] + width * split_ratio
            room1_space = box(bounds[0], bounds[1], split_x, bounds[3])
            room2_space = box(split_x, bounds[1], bounds[2], bounds[3])
        else:
            split_y = bounds[1] + height * split_ratio
            room1_space = box(bounds[0], bounds[1], bounds[2], split_y)
            room2_space = box(bounds[0], split_y, bounds[2], bounds[3])
        ...
        result = {}
        result.update(self._recursive_subdivision(room1_space, rooms1, graph, depth + 1))
        result.update(self._recursive_subdivision(room2_space, rooms2, graph, depth + 1))
    ...
    def _create_shared_wall(self, rooms: Dict[str, Polygon], room_a: str, room_b: str) -> Dict[str, Polygon]:
        ...
        if closest_points:
            ...
            rooms[room_a] = self._extend_to_wall(poly_a, wall_line, is_first=True)
            
            # Adjust room_b to include up to the wall
            rooms[room_b] = self._extend_to_wall(poly_b, wall_line, is_first=False)
    ...
    def _extend_to_wall(self, polygon: Polygon, wall: LineString, is_first: bool) -> Polygon:
        """Extend polygon to meet the wall"""
        # Simple implementation: create new polygon that includes the wall
        coords = list(polygon.exterior.coords)
        
        # Add wall endpoints to polygon
        wall_coords = list(wall.coords)
        
        # Find closest wall point to polygon
        all_coords = coords + wall_coords
        hull_points = self._convex_hull_points(all_coords)
        
        return Polygon(hull_points)
    
    def _convex_hull_points(self, points: List[Tuple]) -> List[Tuple]:
        """Simple convex hull for point set"""
    ...
    def _trim_overlap(self, rooms: Dict[str, Polygon], room_a: str, room_b: str) -> Dict[str, Polygon]:
        ...
        if not overlap.is_empty:
            # Split the overlap between rooms
            overlap_area = overlap.area / 2
            
            # Create cut line
            bounds = overlap.bounds
            cut_x = (bounds[0] + bounds[2]) / 2
            cut_y = (bounds[1] + bounds[3]) / 2
