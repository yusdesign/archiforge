"""
ILP-based floor plan layout solver with proper collision detection
"""
import pulp
import numpy as np
...
class RoomLayoutSolverILP:
    ...
    def solve(self, adjacency_graph: nx.Graph,
        ...
        """Generate non-overlapping floor plan"""
    ...
    def _layout_rectangular(self, rooms: List[str], 
        ...
        for room in sorted_rooms:
            ...
            if x + w > self.config.building_width - 0.5:
                x = 0.5
                y += row_height + 0.2
                row_height = 0
            
            # Ensure within bounds
            if y + h > self.config.building_height - 0.5:
                # Scale down if necessary
                h = self.config.building_height - y - 0.5
    ...
    def _layout_central_hallway(self, rooms: List[str],
        ...
        hallway_y = 1.0
        hallway_height = self.config.building_height - 2.0
        ...
        for room in left_rooms:
            ...
            if current_y + h > hallway_y + hallway_height:
                break
        ...
        right_x = hallway_x + hallway_width + 0.2
        current_y = hallway_y
    ...
    def _layout_grid(self, rooms: List[str],
        ...
        cell_w = (self.config.building_width - 1) / cols
        cell_h = (self.config.building_height - 1) / rows
        ...
        for idx, room in enumerate(rooms):
            ...
            # Fit to cell
            w = min(w, cell_w - margin)
            h = min(h, cell_h - margin)
            w = max(2.0, w)
            h = max(2.0, h)
    ...
    def _layout_recursive(self, rooms: List[str],
        ...
        """Recursive subdivision with size awareness"""
        ...
        building = box(0.5, 0.5, 
                      self.config.building_width - 0.5,
                      self.config.building_height - 0.5)
    ...
    def _layout_compact(self, rooms: List[str],
                        dims: Dict[str, Tuple[float, float]],
                        graph: nx.Graph) -> Dict[str, Polygon]:
        """Compact layout minimizing empty space"""
        result = {}
        ...
        w, h = dims[first]
        result[first] = box(0.5, 0.5, 0.5 + w, 0.5 + h)
        ...
        for room in sorted_rooms[1:]:
            ...
            for placed_room, placed_poly in result.items():
                if graph.has_edge(room, placed_room):
                    ...
                    candidates = [
                        (bounds[2] + 0.1, bounds[1], bounds[2] + 0.1 + w, bounds[1] + h),  # right
                        (bounds[0] - w - 0.1, bounds[1], bounds[0] - 0.1, bounds[1] + h),  # left
                        (bounds[0], bounds[3] + 0.1, bounds[0] + w, bounds[3] + 0.1 + h),  # top
                        (bounds[0], bounds[1] - h - 0.1, bounds[0] + w, bounds[1] - 0.1)   # bottom
                    ]
                    
                    for x1, y1, x2, y2 in candidates:
                        if x1 < 0 or x2 > self.config.building_width or y1 < 0 or y2 > self.config.building_height:
                            continue
            ...
            if best_pos:
                ...
                # Fallback: place in empty corner
                w, h = dims[room]
                result[room] = box(0.5, self.config.building_height - h - 0.5,
                                  0.5 + w, self.config.building_height - 0.5)
