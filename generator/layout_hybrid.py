...
class RoomLayoutSolverHybrid:
    ...
    def _ilp_assign_rooms(self, leaf_nodes: List[KDTreeNode], 
        ...
        # Each leaf gets at most one room (can leave some empty)
        for i in range(n_leaves):
            prob += pulp.lpSum(x[(i, j)] for j in range(n_rooms)) <= 1
        
        # Objective: minimize area difference + maximize adjacency
        area_cost = 0
        for i, leaf in enumerate(leaf_nodes):
            for j, room in enumerate(self.rooms):
        ...
        for u, v in graph.edges():
            u_idx = self.rooms.index(u) if u in self.rooms else -1
            v_idx = self.rooms.index(v) if v in self.rooms else -1
        ...
        # Solve
        solver = pulp.PULP_CBC_CMD(msg=False, timeLimit=self.config.max_depth * 5)
        prob.solve(solver)
        ...
        # Handle unassigned rooms (fallback)
