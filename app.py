...
with st.sidebar:
    ...
    available_rooms = ["Living", "Kitchen", "Bedroom", "Bathroom", "Dining", "Study", "Hallway", "Master Bedroom"]
    ...
    for room in selected_rooms:
        with st.expander(f"{room} Settings"):
            ...
            min_area = col1.number_input(f"Min area ({room})", 4.0, 40.0, 12.0, key=f"min_{room}")
            max_area = col2.number_input(f"Max area ({room})", 8.0, 60.0, 25.0, key=f"max_{room}")
...
if generate_btn and selected_rooms:
    with st.spinner("Generating architecture..."):
        try:
            ...
            # Step 3: Build B-rep with door/window config
...
if st.session_state.generated and st.session_state.rooms:
    ...
    with col2:
        total_area = sum(poly.area for poly in st.session_state.rooms.values() if hasattr(poly, 'is_valid') and poly.is_valid)
        st.markdown(f"""
        <div class="metric-card">
            <h3>📐</h3>
    ...
    with col3:
        ...
        status = "✅ Valid" if validation.get('is_valid', False) else "⚠️ Invalid"
        color = "success-badge" if validation.get('is_valid', False) else "error-badge"
        st.markdown(f"""
        <div class="metric-card">
            <h3>🔍</h3>
    ...
    with tab3:
        ...
        if stats:
            ...
            with col1:
                ...
                st.metric("Floor Area", f"{stats.get('floor_area_m2', 0):.2f} m²")
                st.metric("Room Count", stats.get('room_count', 0))
                st.metric("Wall Length", f"{stats.get('wall_length_m', 0):.1f} m")
            
            with col2:
                st.metric("Interior Walls", f"{stats.get('interior_wall_length_m', 0):.1f} m")
                st.metric("Building Height", f"{stats.get('building_height_m', 0):.2f} m")
                st.metric("Components", stats.get('component_count', 0))
                st.metric("Footprint Area", f"{stats.get('footprint_area_m2', 0):.2f} m²")
    ...
    with tab4:
        ...
        if len(st.session_state.adj_graph.nodes) > 0:
            ...
            with col2:
                if nx.is_connected(st.session_state.adj_graph):
                    diam = nx.diameter(st.session_state.adj_graph)
                    st.metric("Diameter", diam)
                else:
                    st.metric("Diameter", "Disconnected")
            with col3:
